...
so neither needs to be installed separately. It also installs a fish
completion (a static file) unconditionally, and bash/zsh completions
(generated by running the freshly built binary) whenever the build machine
can execute the result, which is true for an ordinary, non-cross build.
...
The fish completion (`completions/rip.fish`) is hand-written rather than
generated, so it can mirror the first-word subcommand rule above and offer
trashed original paths for `restore`/`purge`; the generated bash and zsh
completions do not do either of those two things.
